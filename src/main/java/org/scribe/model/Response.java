...
public class Response {
	...
	Response(HttpURLConnection connection) throws IOException {
		try {
			...
			if (isGZipped(headers))
				stream = isSuccessful() ? new GZIPInputStream(connection.getInputStream()) : connection.getErrorStream();
			else
	...
	private boolean isGZipped(Map<String, String> headers) {
		...
		try {
			...
			String gzipKey = "Content-Encoding";
			String gzipValue = "gzip";

			if (headers.containsKey(gzipKey) && headers.get(gzipKey).equalsIgnoreCase(gzipValue))
				return true;
